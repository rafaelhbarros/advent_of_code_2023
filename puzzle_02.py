puzzle = """Game 1: 2 blue, 3 red; 3 green, 3 blue, 6 red; 4 blue, 6 red; 2 green, 2 blue, 9 red; 2 red, 4 blue
...
def valid_cubes(rounds):
    for count_color in rounds.split(','):
...
def step1():
    ...
    for line in puzzle.splitlines():
        game, shown = line.split(':')
        game_id = game.replace('Game ', '')
        if all(valid_cubes(round) for round in shown.split(';')):
...
def step2():
    ...
    for line in puzzle.splitlines():
        ...
        for round in shown.split(';'):
            for step in round.strip().split(','):
                count, color = step.strip().split()
                min_cubes[color] = max(min_cubes[color], int(count))
        values = list(min_cubes.values())
