...
def part1():
    ...
    parts = 0
    puzzle_lines = list(puzzle.splitlines())
    not_part = {'4', '.', '0', '8', '7', '3', '2', '5', '6', '9', '1'}
    for i, line in enumerate(puzzle_lines):
        for entry in re.finditer(r'\d+', line):
            cmp = int(entry.group())
            x = len(entry.group())
            j = entry.span()[0]
            if set(puzzle_lines[max(0, i-1)][max(j-1, 0):j+x+1]) - not_part:
                parts += int(cmp)  
            elif i + 1 < len(puzzle_lines) and set(puzzle_lines[i+1][max(j-1, 0):j+x+1]) - not_part:
                parts += int(cmp)
            elif j+x+1 < len(line) and set(line[j+x]) - not_part: # right
                parts += int(cmp)
            elif j - 1 > 0 and set(line[j-1]) - not_part:
                parts += int(cmp)
...
def part2():
    ...
    for i, line in enumerate(puzzle_lines):
        for entry in re.finditer(r'\d+', line):
            cmp = int(entry.group())
            length = len(entry.group())
            j = entry.span()[0]
            if '*' in puzzle_lines[max(0, i-1)][max(j-1, 0):j+length+1]:
                x, y = (max(0, i-1), j+puzzle_lines[max(0, i-1)][max(j-1, 0):j+length+1].index('*'))
                if j == 0:
                    gears.append([cmp, (x-1,y), 0])
                else:
                    gears.append([cmp, (x-1,y-1), 0])
            if i + 1 < len(puzzle_lines) and '*' in puzzle_lines[i+1][max(j-1, 0):j+length+1]:
                x, y = (i+1, j+puzzle_lines[i+1][max(j-1, 0):j+length+1].index('*'))
                gears.append([cmp, (x-1,y-1), 0])
            if j+length+1 < len(line) and '*' == line[j+length]: #right
                x, y = (i, j+length)
                gears.append([cmp, (x-1,y), 0])
            if j - 1 > 0 and line[j-1] == '*': #left
                x, y = (i, j-1)
                gears.append([cmp, (x-1,y), 0])
    total = 0
